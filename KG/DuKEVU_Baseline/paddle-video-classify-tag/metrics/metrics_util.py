    ...
    def finalize_and_log_out(self,
        ...
        if self.mode == 'infer':
            ...
            for index, item in enumerate(self.infer_results):
                video_id = item[0]
                class_id = item[1][0]
                top1_results[video_id.split(".")[0]] = {
                    "class_id": class_id,
                    "class_name": fl[class_id].strip(),
                    "probability": item[2][0],
                }
            with io.open(
                    os.path.join(savedir, savefile), "w",
                    encoding="utf-8") as ouf:
                json.dump(top1_results, ouf, ensure_ascii=False)
            logger.info("Saved " + os.path.join(savedir, savefile))
        else:
    ...
    def finalize_and_log_out(self,
        ...
        if self.mode == 'infer':
            ...
            with io.open(
                    os.path.join(savedir, savefile), "w",
                    encoding="utf-8") as ouf:
                json.dump(top1_results, ensure_ascii=False)
            logger.info("Saved " + os.path.join(savedir, savefile))
        else:
