...
class AttentionLSTM(ModelBase):
    ...
    def build_input(self, use_dataloader):
        ...
        for name, dim in zip(self.feature_names, self.feature_dims):
            self.feature_input.append(
                paddle.static.data(
                    shape=[None, dim], lod_level=1, dtype='float32',
                    name=name))
        if self.mode != 'infer':
    ...
    def build_model(self):
        ...
        self.logit = paddle.static.nn.fc(x=fc2, size=self.num_classes, activation=None, \
    ...
    def load_pretrain_params(self, exe, pretrain, prog):
        ...
        state_dict = paddle.fluid.load_program_state(pretrain)
        ...
        for name in dict_keys:
            if "output" in name:
                ...
                logger.info(
                    ...
                    format(name))
        paddle.fluid.set_program_state(prog, state_dict)
