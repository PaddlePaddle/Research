...
def parse_args():
    ...
    parser.add_argument(
        '--save_dir',
        type=str,
        default=os.path.join('CCSK_dataset', 'tsn_features'),
...
def infer(args):
    ...
    for infer_iter, data in enumerate(infer_reader()):
        ...
        t_start = t_end
        t_end = timeit.default_timer()
        logger.info("[%s][%s] Load: %.2f\tExec: %.2f" %
                    (video_id[0].split('/')[-1][:-4],
                     str(datetime.now()).split(" ")[1].split(".")[0],
                     load_time, t_end - t_start))
        if (infer_iter + 1) % 20 == 0:
            logger.info("[%s] Processed %d .." % (str(datetime.now()),
                                                  infer_iter + 1))
