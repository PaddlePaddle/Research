...
def log_lr_and_step():
    try:
        ...
        lr_var = paddle.static.global_scope().find_var("learning_rate")
        if not lr_var:
            lr_var = paddle.static.global_scope().find_var("learning_rate_0")
        lr = np.array(lr_var.get_tensor())

        lr_count = '[-]'
        lr_count_var = paddle.static.global_scope().find_var(
            "@LR_DECAY_COUNTER@")
        if lr_count_var:
...
def test_with_dataloader(exe,
    ...
    for data in test_dataloader():
        ...
        test_iter += 1
    test_metrics.finalize_and_log_out("[TEST] Epoch{} Finish".format(epoch))
    ...
    for epoch in range(epochs):
        ...
        for data in train_dataloader():
            ...
            if is_profiler and epoch == 0 and train_iter == log_interval:
                ...
                paddle.fluid.profiler.stop_profiler("total", profiler_path)
                return
        ...
        if compiled_test_prog and valid_interval > 0 and (
            ...
            test_with_dataloader(exe, compiled_test_prog, test_dataloader,
                                 test_fetch_list, test_metrics, epoch,
                                 log_interval, save_model_name)
...
def save_model(exe, program, save_dir, model_name, postfix=''):
    ...
    paddle.fluid.save(program, os.path.join(save_dir, saved_model_name))
