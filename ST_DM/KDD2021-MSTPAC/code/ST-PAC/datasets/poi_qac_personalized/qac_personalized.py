        ...
        if self.is_training:
            ...
            if self._flags.use_geohash:
                inputs['neg_loc_geoid'] = fluid.layers.data(name="neg_loc_geoid", shape=[40],
                    dtype="int64", lod_level=0)
            inputs['day_id'] = fluid.layers.data(name="day_id", shape=[14],
                    dtype="int64", lod_level=0)
        else:
            #for predict label
        ...
        for pos_id in pos_ids:
            ...
            if self.is_training:
                ...
                if self._flags.reader_batch:
                    ...
                    yield prefix_input + pos_input + neg_input + [("day_id", day_input)]
                else:
                    for neg_id in nids_sample:
                ...
                yield prefix_input + pos_input + [("label", [label]), ("qid", [logid]), ("day_id", day_input)]
