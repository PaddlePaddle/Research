...
"""
 Specify the brief transformer.py
 Modify: map(zhuoan@baidu.com)
...
def _wrap_layer_with_block(layer, block_idx):
    """
    ...
    def _layer_wrapper(*args, **kwargs):
...
def _scaled_dot_product_attention(q, k, v, attn_bias, d_key, dropout_rate):
    """
...
def _combine_heads(x):
    ...
    so that it becomes one dimension, which is reverse to _split_heads.
    """
    if len(x.shape) != 4:
...
def multi_head_attention(queries,
    ...
    def __compute_qkv(queries, keys, values, n_head, d_key, d_value):
        ...
        fc_layer = _wrap_layer_with_block(
    ...
    ctx_multiheads = _scaled_dot_product_attention(q, k, v, attn_bias, d_model,
                                                  dropout_rate)
    out = _combine_heads(ctx_multiheads)
...
def _positionwise_feed_forward(x, d_inner_hid, d_hid, dropout_rate):
    """
...
def _pre_post_process_layer(prev_out, out, process_cmd, dropout_rate=0.):
    """
...
pre_process_layer = functools.partial(_pre_post_process_layer, None)
post_process_layer = _pre_post_process_layer


def _prepare_encoder_decoder(src_word,
                            src_pos,
                            src_vocab_size,
                            src_emb_dim,
                            src_max_len,
...
prepare_encoder = functools.partial(
    _prepare_encoder_decoder, pos_enc_param_name=pos_enc_param_names[0])
prepare_decoder = functools.partial(
    _prepare_encoder_decoder, pos_enc_param_name=pos_enc_param_names[1])


def _encoder_layer(enc_input,
                  attn_bias,
                  n_head,
                  d_key,
                  d_value,
    ...
    attn_output = post_process_layer(enc_input, attn_output, postprocess_cmd,
                                     prepostprocess_dropout)
    ffd_output = _positionwise_feed_forward(
...
def _do_beam_search(trg_vocab_size,
                    max_in_len,
                    n_layer,
                    n_head,
                    d_key,
                    ...
                    bos_idx,
                    eos_idx,
                    ids, scores, parent_idx, trg_src_attn_bias,
                    caches, enc_output, step_idx):
    """
        do beam search
    """
...
def fast_decode(src_vocab_size,
    ...
    _do_beam_search(trg_vocab_size,
            max_in_len,
            n_layer,
            n_head,
            d_key,
