...
from nets.common.transformer_encoder import encoder, pre_process_layer
from nets.common.transformer_encoder import gelu
...
class ErnieModel(object):
    ...
    def __init__(self,
        ...
        self._freeze_emb = config.get('freeze_emb', False)
        self._freeze_num_layers = config.get('freeze_num_layers', 0)
        ...
        if src_ids and position_ids and sentence_ids and task_ids and input_mask:
            self._build_model(src_ids, position_ids, sentence_ids, task_ids,
                            input_mask)
    
    def freeze_embedding(self):
        """freeze embedding"""
        embedding_name = ["word_embedding", "sent_embedding", "pos_embedding"] 
        embedding_name += ["pre_encoder_layer_norm_bias", "pre_encoder_layer_norm_scale"]
        if self._use_task_id:
            embedding_name += ["task_embedding"]
        for emb_name in embedding_name:
            embedding = fluid.default_main_program().global_block().var(emb_name)
            embedding.stop_gradient = True 

    def verbose_config(self):
        """ verbose ernie config """
        logging.info('Configuration:')
        logging.info('    self._emb_size : {} '.format(self._emb_size))
        logging.info('    self._hidden_size : {} '.format(self._hidden_size))
        logging.info('    self._n_layer : {} '.format(self._n_layer))
        logging.info('    self._n_head : {} '.format(self._n_head))
        logging.info('    self._voc_size : {} '.format(self._voc_size))
        logging.info('    self._max_position_seq_len : {} '.format(self._max_position_seq_len))
        logging.info('    self._param_share : {} '.format(self._param_share))
        logging.info('    self._pre_encoder_cmd : {} '.format(self._pre_encoder_cmd))
        logging.info('    self._preprocess_cmd : {} '.format(self._preprocess_cmd))
        logging.info('    self._postprocess_cmd : {} '.format(self._postprocess_cmd))
        logging.info('    self._epsilon : {} '.format(self._epsilon))
        logging.info('    self._emb_mapping_in : {} '.format(self._emb_mapping_in))
        logging.info('    self._n_layer_per_block : {} '.format(self._n_layer_per_block))
        logging.info('    self._sent_types : {} '.format(self._sent_types))
        if self._use_task_id:
            logging.info('    using task_id, #task_types:{}'.format(self._task_types))
        else:
            logging.info('    using task_id, #task_types:{}'.format(self._use_task_id))
        logging.info('    self._hidden_act : {} '.format(self._hidden_act ))
        logging.info('    self._prepostprocess_dropout : {} '.format(self._prepostprocess_dropout))
        logging.info('    self._attention_dropout : {} '.format(self._attention_dropout))
        logging.info('    self._weight_sharing : {}'.format(self._weight_sharing))
    ...
    def _build_model(self, src_ids, position_ids, sentence_ids, task_ids,
        ...
        self._enc_out, self._checkpoints= encoder(
            ...
            freeze_num_layers=self._freeze_num_layers)
        if self._dtype == "float16":
    ...
    def check_nessesary_fields(self, reader):
        """ check ernie fields """
        self.fields = ["src_ids", "pos_ids", "sent_ids", "task_ids", "input_mask"]
        reader_backbone_name = reader.data_types.keys()
        for name in self.fields:
            if name not in reader_backbone_name:
                raise Exception("Invalid backbone name !", name)
