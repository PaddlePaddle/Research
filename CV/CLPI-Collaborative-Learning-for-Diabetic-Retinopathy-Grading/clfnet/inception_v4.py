...
class InceptionV4():
    ...
    def net(self, input, class_dim=1000, lesion_map=None):
        ...
        ### Modified by shangfangxin@baidu.com @ 2019-9-20 ###
        # x.shape = 14,14,1536 -concat-> 14,14,1537 -conv1x1-> 14,14,1536
        if lesion_map is not None:
            if not isinstance(lesion_map, str):
                lesion_map = fluid.layers.image_resize(lesion_map, out_shape=(14, 14), resample="NEAREST")
                x = fluid.layers.concat([x, lesion_map], axis=1, name='lesion_concat')
            x = fluid.layers.conv2d(x, num_filters=1536,
                                       filter_size=1,
                                       stride=1,
                                       padding=0,
                                       groups=1,
