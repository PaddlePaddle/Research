...
class DenseNet():
    ...
    ### Modified by shangfangxin@baidu.com @ 2019-9-10 ###
    def net(self, input, bn_size=4, dropout=0, class_dim=1000, lesion_map=None, CAM=False):
        layers = self.layers
        ...
        if lesion_map is not None:
            if not isinstance(lesion_map, str):
                conv = fluid.layers.concat([conv, lesion_map], axis=1, name='lesion_concat')
