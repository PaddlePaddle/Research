...
def resize(img, size, interpolation=cv2.INTER_LINEAR):
    ...
    if isinstance(size, int):
        ...
        if w < h:
            ow = size
            oh = int(size * h / w)
            output = cv2.resize(img, dsize=(ow, oh),
...
def pad(img, padding, fill=0, padding_mode='constant'):
    ...
    if not _is_numpy_image(img):
        raise TypeError(
            'img should be numpy ndarray. Got {}'.format(type(img)))
...
def to_grayscale(img, num_output_channels=1):
    ...
    if num_output_channels == 1:
        ...
        img = np.broadcast_to(cv2.cvtColor(img, cv2.COLOR_RGB2GRAY)[
                              :, :, np.newaxis], img.shape)
    return img


def radius_reduction(img, mask):
    '''
    叠加Mask，去除中值滤波灰色外缘
    '''
