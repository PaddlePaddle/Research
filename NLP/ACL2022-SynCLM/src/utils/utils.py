...
IMPORT_COUNT = 0
def visualdl_log(metrics_output, train_mlm_loss, train_att_loss, train_contrast_loss, train_total_loss, steps, phase):
    """log可视化，仅限于paddlecloud 平台任务
    """
    print(
        "{phase} log: steps {steps}, train_mlm_loss {train_mlm_loss}, train_att_loss {train_att_loss}, train_contrast_loss {train_contrast_loss}, train_total_loss {train_total_loss}, metrics: {metrics}"
        .format(phase=phase,
                steps=steps,
                train_mlm_loss=train_mlm_loss,
                train_att_loss=train_att_loss,
                train_contrast_loss=train_contrast_loss,
