...
def apply_dynamic_loss_scaling(loss_scaling, master_params_grads, incr_every_n_steps, decr_every_n_nan_or_inf,
                               incr_ratio, decr_ratio):
    _incr_every_n_steps = fluid.layers.fill_constant(shape=[1], dtype='int32', value=incr_every_n_steps)
    _decr_every_n_nan_or_inf = fluid.layers.fill_constant(shape=[1], dtype='int32', value=decr_every_n_nan_or_inf)

    _num_good_steps = fluid.layers.create_global_var(name=fluid.unique_name.generate("num_good_steps"),
                                                     shape=[1],
                                                     value=0,
                                                     dtype='int32',
                                                     persistable=True)
    _num_bad_steps = fluid.layers.create_global_var(name=fluid.unique_name.generate("num_bad_steps"),
                                                    shape=[1],
                                                    value=0,
                                                    dtype='int32',
                                                    persistable=True)
    ...
    update_loss_scaling(is_overall_finite, loss_scaling, _num_good_steps, _num_bad_steps, _incr_every_n_steps,
...
def update_loss_scaling(is_overall_finite, prev_loss_scaling, num_good_steps, num_bad_steps, incr_every_n_steps,
    ...
    with fluid.layers.Switch() as switch:
        with switch.case(is_overall_finite):
            should_incr_loss_scaling = fluid.layers.less_than(incr_every_n_steps, num_good_steps + 1)
            with fluid.layers.Switch() as switch1:
                with switch1.case(should_incr_loss_scaling):
                    ...
                    loss_scaling_is_finite = fluid.layers.isfinite(new_loss_scaling)
                    with fluid.layers.Switch() as switch2:
                        with switch2.case(loss_scaling_is_finite):
                            fluid.layers.assign(new_loss_scaling, prev_loss_scaling)
        ...
        with switch.default():
            should_decr_loss_scaling = fluid.layers.less_than(decr_every_n_nan_or_inf, num_bad_steps + 1)
                ...
                with switch3.case(should_decr_loss_scaling):
                    ...
                    static_loss_scaling = \
                        fluid.layers.fill_constant(shape=[1],
                                             dtype='float32',
                                             value=1.0)
                    less_than_one = fluid.layers.less_than(new_loss_scaling, static_loss_scaling)
                    with fluid.layers.Switch() as switch4:
                        with switch4.case(less_than_one):
                            fluid.layers.assign(static_loss_scaling, prev_loss_scaling)
