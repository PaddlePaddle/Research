...
from model.synclm import SynCLMConfig
...
from utils.utils import visualdl_log, get_time
...
def main(args):
    ernie_config = SynCLMConfig(args.synclm_config_path)
    ...
    if args.do_train:
        ...
        train_data_generator = data_reader.data_generator(input_file=args.train_set,
        ...
        with fluid.program_guard(train_program, startup_prog):
            with fluid.unique_name.guard():
                ...
                scheduled_lr, loss_scaling = optimization(loss=graph_vars["loss"],
        ...
        if args.verbose:
            if args.in_tokens:
                lower_mem, upper_mem, unit = fluid.contrib.memory_usage(program=train_program,
                                                                        batch_size=args.batch_size // args.max_seq_len)
            else:
                lower_mem, upper_mem, unit = fluid.contrib.memory_usage(program=train_program,
                                                                        batch_size=args.batch_size)
            print("Theoretical memory usage in training: %.3f - %.3f %s" % (lower_mem, upper_mem, unit))

    if args.do_val or  args.do_testor args.do_pred:
        test_prog = fluid.Program()
        with fluid.program_guard(test_prog, startup_prog):
            with fluid.unique_name.guard():
                test_pyreader, graph_vars = create_model(args, pyreader_name='test_reader', ernie_config=ernie_config)
    ...
    if args.do_train:
        ...
        if args.init_checkpoint:
            init_checkpoint(exe, args.init_checkpoint, main_program=train_program)
        elif args.init_pretraining_params:
            init_pretraining_params(exe, args.init_pretraining_params, main_program=train_program)
    ...
    if args.do_val or args.do_test or args.do_pred:
        if args.use_multi_gpu_test:
            test_exe = fluid.ParallelExecutor(use_cuda=args.use_cuda, main_program=test_prog, share_vars_from=train_exe)
        ...
        while True:
            try:
                ...
                if steps % skip_steps == 0:
                    train_fetch_list = [
                        graph_vars["loss"].name, graph_vars["accuracy"].name, graph_vars["num_seqs"].name
                    ]
                    ...
                    print("%s - epoch: %d, progress: %d/%d, step: %d, ave loss: %f, speed: %f steps/s" % \
                          (get_time(), current_epoch, current_example, num_train_examples, steps, outputs["loss"], args.skip_steps / used_time))
                    time_begin = time.time()
                else:
