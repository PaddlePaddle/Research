...
from model.synclm import SynCLMModel
...
def create_model(args, pyreader_name, ernie_config, is_prediction=False):
    ...
    synclm = SynCLMModel(emb_ids=emb_ids, input_mask=input_mask, config=ernie_config)

    cls_feats = synclm.get_pooled_output()
    ...
    logits = fluid.layers.fc(input=cls_feats,
                             ...
                             param_attr=fluid.ParamAttr(name=cls_params_name[0],
                                                        initializer=fluid.initializer.TruncatedNormal(scale=0.02)),
                             bias_attr=fluid.ParamAttr(name=cls_params_name[1],
                                                       initializer=fluid.initializer.Constant(0.)))

    if is_prediction:
        probs = fluid.layers.softmax(logits)
        feed_targets_name = [src_ids.name, pos_ids.name, sent_ids.name, input_mask.name]
...
def evaluate(args, exe, test_program, test_pyreader, graph_vars, eval_phase):
    ...
    fetch_list = [
        graph_vars["loss"].name, graph_vars["probs"].name, graph_vars["labels"].name, graph_vars["num_seqs"].name,
    ...
    while True:
        try:
            ...
            total_cost += np.sum(np_loss * len(np_loss))
            total_num_seqs += np.sum(len(np_loss))
