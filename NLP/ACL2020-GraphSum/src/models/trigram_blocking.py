...
class TrigramBlocking(object):
    ...
    def __init__(self, init_token, spm_tokenizer, use_fp16, beam_size):
        ...
        self.spm_tokenizer = spm_tokenizer
        id2is_full_token = self._build_id2is_full_token(spm_tokenizer, self._dtype)
        self._id2is_full_token = fluid.layers.create_parameter(shape=id2is_full_token.shape, dtype=self._dtype,
    ...
    def _build_id2is_full_token(self, spm_tokenizer, dtype):
        vocab_sz = len(spm_tokenizer)
        is_full_token = [0.0] * vocab_sz

        for token_id in range(vocab_sz):
            token = spm_tokenizer.id_to_piece(token_id)
            if token.startswith('▁'):
