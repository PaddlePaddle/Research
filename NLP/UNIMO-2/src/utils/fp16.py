...
def apply_dynamic_loss_scaling(loss_scaling, master_params_grads,
    ...
    _incr_every_n_steps = paddle.full(
        shape=[1], fill_value=incr_every_n_steps)
    _decr_every_n_nan_or_inf = paddle.full(
        shape=[1], fill_value=decr_every_n_nan_or_inf)

    _num_good_steps = paddle.fluid.layers.create_global_var(
        name=paddle.utils.unique_name.generate("num_good_steps"),
        shape=[1],
        value=0,
        dtype='int32',
        persistable=True)
    _num_bad_steps = paddle.fluid.layers.create_global_var(
    ...
    grads = [paddle.sum(x=g) for [_, g] in master_params_grads]
    all_grads = paddle.concat(x=grads)
    all_grads_sum = paddle.sum(x=all_grads)
    ...
    with paddle.fluid.layers.Switch() as switch:
        with switch.case(is_overall_finite):
            pass
        with switch.default():
            for _, g in master_params_grads:
                paddle.assign(paddle.zeros_like(x=g), g)
...
def update_loss_scaling(is_overall_finite, prev_loss_scaling, num_good_steps,
    ...
    Args:
        ...
        decr_ratio(float): The less-than-one-multiplier to use when decreasing 
                           loss scaling.
    """
    zero_steps = paddle.full(shape=[1], fill_value=0)
    with paddle.fluid.layers.Switch() as switch:
        with switch.case(is_overall_finite):
            should_incr_loss_scaling = paddle.less_than(
                x=incr_every_n_steps, y=num_good_steps + 1)
            with paddle.fluid.layers.Switch() as switch1:
                with switch1.case(should_incr_loss_scaling):
                    new_loss_scaling = prev_loss_scaling * incr_ratio
                    loss_scaling_is_finite = paddle.isfinite(
                        new_loss_scaling)
                    with paddle.fluid.layers.Switch() as switch2:
                        with switch2.case(loss_scaling_is_finite):
                            paddle.assign(new_loss_scaling,
                                                prev_loss_scaling)
                        with switch2.default():
                            pass
                    paddle.assign(zero_steps, num_good_steps)
                    paddle.assign(zero_steps, num_bad_steps)

                with switch1.default():
                    paddle.increment(x=num_good_steps)
                    paddle.assign(zero_steps, num_bad_steps)

        with switch.default():
            should_decr_loss_scaling = paddle.less_than(
                x=decr_every_n_nan_or_inf, y=num_bad_steps + 1)
            with paddle.fluid.layers.Switch() as switch3:
                with switch3.case(should_decr_loss_scaling):
                    new_loss_scaling = prev_loss_scaling * decr_ratio
                    static_loss_scaling = \
                        paddle.full(shape=[1],
                                             fill_value=1.0)
                    less_than_one = paddle.less_than(x=new_loss_scaling,
                                                           y=static_loss_scaling)
                    with paddle.fluid.layers.Switch() as switch4:
                        with switch4.case(less_than_one):
                            paddle.assign(static_loss_scaling,
                                                prev_loss_scaling)
                ...
                with switch3.default():
                    paddle.assign(zero_steps, num_good_steps)
                    paddle.increment(x=num_bad_steps)
