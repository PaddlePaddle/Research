...
def scale_l2(x, norm_length):
    ...
    alpha = paddle.max(x=paddle.abs(x), axis=1, keepdim=True) + 1e-12
    l2_norm = alpha * paddle.sqrt(
        paddle.sum(x=paddle.pow(x=x / alpha), axis=1, keepdim=True) + 1e-6)
    x_unit = x / l2_norm
...
def pgd_loss(ernie, labels, loss, task_fc_fn, epsilon=0.25):
    ...
    param_grads = paddle.static.append_backward(loss, parameter_list=[ernie._word_emb_name])
    ...
    d = paddle.fluid.layers.gaussian_random(emb.shape)
