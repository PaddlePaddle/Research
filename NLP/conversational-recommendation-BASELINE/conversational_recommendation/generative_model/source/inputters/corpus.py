...
class KnowledgeCorpus(object):
    ...
    def __init__(self,
        ...
        self.filter_pred = lambda ex: filter_pred(ex['src']) and filter_pred(ex['tgt']) #src --> G + X + KG; tgt --> Y; knowledge --> KG
    ...
    def read_data(self, data_file):
        ...
        with open(data_file, "r") as f:
            for line in f: 
                if line.rstrip('\n').split('\t') < 4: 
                    continue
                src, history, tgt, knowledge = line.rstrip('\n').split('\t')[:4] #src --> G; history --> X; tgt --> Y; knowledge --> KG
                filter_knowledge = []
                for sent in knowledge.split('\1'):
                    filter_knowledge.append(' '.join(sent.split()[: self.max_len]))
                data.append({'src': src, 'his': history, 'tgt': tgt, 'cue':filter_knowledge})
        return data
    ...
    def build_examples(self, data):
        ...
        for raw_data in data:
            ...
            if not self.filter_pred(example): 
                continue
            examples.append((example['src'], example['his'], example['tgt'], example['cue'])) #src --> G + X + KG; tgt --> Y; cue --> KG
        return examples
    ...
    def preprocessing_for_lines(self, lines, batch_size):
        ...
        for line in lines:
            ...
            for sent in knowledge.split('\1'):
                filter_knowledge.append(' '.join(sent.split()[: self.max_len]))
            raw_data.append({'src': src, 'his': history, 'tgt': tgt, 'cue': filter_knowledge}) #src --> G + X + KG; tgt --> Y; cue --> KG
        ...
        def instance_reader():
            ...
            for (index, example) in enumerate(examples):
                instance = [example[0], example[1], example[2], example[3]]
                yield instance
    ...
    def prepare_batch_data(self, batch): 
        ...
        batch_history_ids = [inst[1] for inst in batch]
        batch_target_ids = [inst[2] for inst in batch]
        batch_knowledge_ids = [inst[3] for inst in batch]

        pad_source = max([self.cal_max_len(s_inst) for s_inst in batch_source_ids])
        pad_history = max([self.cal_max_len(s_inst) for s_inst in batch_history_ids])
        ...
        history_pad_ids = [self.pad_data(h_inst, pad_history) for h_inst in batch_history_ids]
        ...
        history_len = [len(inst) for inst in batch_history_ids]
        ...
        return_array = [np.array(source_pad_ids).reshape(-1, pad_source), np.array(source_len),
                        np.array(history_pad_ids).reshape(-1, pad_history), np.array(history_len),
