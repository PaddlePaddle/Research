...
import math
import subprocess
from eval.tokenization import BasicTokenizer
...
class GenerationEval(object):
    ...
    def __init__(self, args):
        ...
        self.eval_script = eval_script.split(" ")
        self.eval_mertrics = ["rouge-1","rouge-2","rouge-l"]
        self.basic_tokenizer = BasicTokenizer(do_lower_case=True)
        # self.roberta_tokenizer = GptBpeTokenizer(vocab_file=args.roberta_vocab_file,
    ...
    def eval(self, output_file, phase="", features=None):
        ...
        eval_res = {}
        # print("1111")
        # print(output_file.strip("."))
        # print(phase.strip("."))
        if self.eval_script:
            eval_res = subprocess.check_output(self.eval_script + [output_file.strip("."), phase.strip(".")])
            # print("here" + str(eval_res))
    ...
    def direct_eval(self, preds, refs):
        """
        direct eval rouge score
        """
        eval_res = {}
        final_preds = []
        final_refs = []
        for pred in preds:
            final_preds.append(self.basic_tokenizer.tokenize(pred.strip()))
        for ref in refs:
            final_refs.append(self.basic_tokenizer.tokenize(ref.strip()))

        for mertric in self.eval_mertrics:
            eval_func = getattr(self, mertric, None)
            if eval_func:
                eval_res[mertric] = eval_func(final_refs, final_preds)
        ret = []
        for mertric in self.eval_mertrics:
            mertric_res = eval_res.get(mertric, None)
            if mertric_res is None:
