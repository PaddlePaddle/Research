...
model_g.add_arg("use_interval", bool, False, "whether model interval")
model_g.add_arg("roberta_config_path", str, "roberta_config/roberta_config.json",
                                "The file to save roberta configuration.")

train_g = ArgumentGroup(parser, "training", "training options.")
train_g.add_arg("pos_win", float, 2.0, "the parameter in graph attention.")
...
train_g.add_arg("learning_rate", float, 3.0, "Learning rate used to train with warmup.")
train_g.add_arg("lr_scheduler", str, "linear_warmup_decay",
                "scheduler of learning rate.", choices=['linear_warmup_decay', 'noam_decay'])
train_g.add_arg("weight_decay", float, 0.01, "Weight decay rate for L2 regularizer.")
train_g.add_arg("warmup_proportion", float, 0.1,
                "Proportion of training steps to perform linear learning rate warmup for.")
train_g.add_arg("warmup_steps", int, 8000, "The training steps to perform linear learning rate warmup.")
train_g.add_arg("eps", float, 1e-9, "epsilon for Adam optimizer")
...
train_g.add_arg("block_trigram", bool, True, "remove repeated trigrams in summary.")
...
data_g.add_arg("max_para_len", int, 512, "Number of words in longest paragraph.")
data_g.add_arg("max_para_num", int, 30, "Number of paragraphs of the longest documents.")
data_g.add_arg("max_doc_num", int, 20, "Number of docs of the multi-doc articles.")
data_g.add_arg("max_tgt_len", int, 300, "the number of tokens in target summaries.")
data_g.add_arg("graph_type", str, "similarity", "type of graph", choices=["similarity", "topic", "discourse"])
data_g.add_arg("batch_size", int, 100, "Total examples' number in batch for training. see also --in_tokens.")
