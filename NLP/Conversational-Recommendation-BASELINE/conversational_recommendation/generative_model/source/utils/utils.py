...
def pad_to_bath_size(src_ids, src_len, his_ids, his_len, trg_ids, trg_len, kn_ids, kn_len, batch_size):
    ...
    new_his_ids = pad(his_ids)
    new_his_len = pad(his_len)
    new_trg_ids = pad(trg_ids)
    new_trg_len = pad(trg_len)
    new_kn_ids = pad(kn_ids)
    new_kn_len = pad(kn_len)

    return [new_src_ids, new_src_len, new_his_ids, new_his_len, new_trg_ids, new_trg_len, new_kn_ids, new_kn_len]
...
def build_data_feed(data, place,
    ...
    """ build data feed """
    src_ids, src_len, his_ids, his_len, trg_ids, trg_len, kn_ids, kn_len = data

    real_size = src_ids.shape[0]
    if src_ids.shape[0] < batch_size:
        if not is_training:
            src_ids, src_len, his_ids, his_len, trg_ids, trg_len, kn_ids, kn_len = \
                pad_to_bath_size(src_ids, src_len, his_ids, his_len, trg_ids, trg_len, kn_ids, kn_len, batch_size)
        else:
            return None
    ...
    goal_input = np.expand_dims(his_ids[:, 1: -1], axis=2)
    goal_mask = len_to_mask(his_len - 2)
    ...
    if not is_training:
        return {'enc_input': to_lodtensor(enc_input, src_len - 2, place),
                'enc_mask': enc_mask,
                'goal_input': to_lodtensor(goal_input, his_len - 2, place),
                #'goal_mask': goal_mask,
