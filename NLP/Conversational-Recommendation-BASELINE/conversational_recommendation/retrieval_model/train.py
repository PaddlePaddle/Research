...
def create_model(args, num_labels, is_prediction=False):
    """create_model"""
    context_ids = fluid.layers.data(name='context_ids', \
        shape=[-1, args.max_seq_len, 1], dtype='int64', lod_level=0)
    context_pos_ids = fluid.layers.data(name='context_pos_ids', \
        shape=[-1, args.max_seq_len, 1], dtype='int64', lod_level=0)
    context_segment_ids = fluid.layers.data(name='context_segment_ids', \
        shape=[-1, args.max_seq_len, 1], dtype='int64', lod_level=0)
    context_attn_mask = fluid.layers.data(name='context_attn_mask', \
        shape=[-1, args.max_seq_len, args.max_seq_len], dtype='float', lod_level=0)
    labels = fluid.layers.data(name='labels', \
        shape=[1], dtype='int64', lod_level=0)
    context_next_sent_index = fluid.layers.data(name='context_next_sent_index', \
        shape=[1], dtype='int64', lod_level=0)
    if "kn" in args.task_name: 
        kn_ids = fluid.layers.data(name='kn_ids', \
            shape=[1], dtype='int64', lod_level=1)
        goal_ids = fluid.layers.data(name='goal_ids', \
            shape=[1], dtype='int64', lod_level=1)
        feed_order = ["context_ids", "context_pos_ids", "context_segment_ids", \
        "context_attn_mask", "kn_ids", "goal_ids", "labels", "context_next_sent_index"]
    else: 
        kn_ids = None
        goal_ids = None
        feed_order = ["context_ids", "context_pos_ids", "context_segment_ids", \
        "context_attn_mask", "labels", "context_next_sent_index"]
