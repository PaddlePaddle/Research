...
from source.encoders.transformer import encoder
from source.encoders.transformer import pre_process_layer
...
class RetrievalModel(object):
    ...
    def __init__(self,
        ...
        self._build_model(context_ids, context_pos_ids, \
                context_segment_ids, context_attn_mask, kn_ids, goal_ids)

    def _build_memory_network(self, kn_ids, rnn_hidden_size=128): 
        """_build_memory_network"""
    ...
    def _build_model(self, 
        ...
        if kn_ids: 
            ...
            self.goal_encoder_out, self.goal_encoder_proj_out = \
                    self._build_memory_network(goal_ids)
    ...
    def get_context_response_memory(self, context_next_sent_index): 
        """get_context_response_memory"""
        context_out = self.get_pooled_output(context_next_sent_index)
        #goal_out = self.get_pooled_output(context_next_sent_index) #need modify
        goal_out = fluid.layers.sequence_pool(input=self.goal_encoder_out, pool_type='sum')
        context_goal_out = fluid.layers.concat(input=[context_out, 
                goal_out], axis=1)
        context_goal_out = fluid.layers.fc(input=context_goal_out,
                    size=256,
                    bias_attr=False)
        kn_context = self.attention(context_goal_out, \
                self.memory_encoder_out, self.memory_encoder_proj_out)
        #cls_feats = fluid.layers.concat(input=[context_out, kn_context], axis=1)
        cls_feats = fluid.layers.concat(input=[context_out, kn_context, goal_out], axis=1)
        return cls_feats
