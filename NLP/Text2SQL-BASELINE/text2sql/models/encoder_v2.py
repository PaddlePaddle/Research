...
"""encoder for text2sql task

Filname: encoder.py
Authors: ZhangAo(@baidu.com)
Date: 2021-01-27 11:55:48
"""
...
class Text2SQLEncoderV2(nn.Layer):
    ...
    def __init__(self, model_config, extra=None):
        ...
        if model_config.pretrain_model_type == 'BERT':
            ...
            vocab_file = os.path.join(os.path.expandvars('$HOME'),
                                     '.paddlenlp/models',
            ...
            with open(Path(model_config.pretrain_model) / 'ernie_config.json') as ifs:
        ...
        if not self.rel_has_value:
            self.value_align = relational_transformer.RelationalPointerNet(hidden_size=self.hidden_size, num_relations=0)
    ...
    def forward(self, inputs):
        ...
        Raises: NULL
        """
        cls_hidden, seq_hidden = self.base_encoder(inputs['src_ids'], inputs['sent_ids'])
        ...
        column_pointer_maps = [
            {i: [i] for i in range(len(orig_input.columns))}
            for orig_input in orig_inputs
        ]
        table_pointer_maps = [
            {i: [i] for i in range(len(orig_input.tables))}
            for orig_input in orig_inputs
        ]
        value_pointer_maps = [
            {i: [i] for i in range(len(orig_input.values))}
            for orig_input in orig_inputs
        ]
        ...
        for batch_idx, orig_input in enumerate(orig_inputs):
            ...
            enc_results.append(EncoderState(state = None,
    ...
    def span_encoder(self, cls_hidden, seq_hidden, span_index, span_tokens_index, span_tokens_mask, proj_fn=None):
        ...
        span_tokens_enc_origin = paddle.reshape(span_tokens_enc_origin, [-1, max_col_nums, max_col_tokens, hidden_size])
        span_enc2 = paddle.sum(
                paddle.multiply(span_tokens_enc_origin, span_tokens_weight.unsqueeze([-1])),
                axis=2)
