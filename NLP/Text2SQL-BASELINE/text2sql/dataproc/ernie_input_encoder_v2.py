...
ErnieInput = namedtuple("ErnieInput", "token_ids sent_ids table_indexes column_indexes value_indexes value_list token_mapping orig_question_tokens candi_nums")
...
class ErnieInputEncoderV2(BaseInputEncoder):
    ...
    def encode(self, question, db, column_match_cells=None, candi_nums=None, col_orders=None, debug=False):
        ...
        return ErnieInput(token_ids, sent_ids, table_indexes, column_indexes, value_indexes,
                          value_list, token_mapping, orig_question_tokens, candi_nums)
    ...
    def tokenize(self, question, db, column_match_cells=None, candi_nums=None, candi_nums_index=None, col_orders=None):
            ...
            if match_cells is not None and len(match_cells) > 0:
                if column.dtype in ('text', 'time'):
                    if not self.config.predict_value:
                        match_cells = match_cells[:1]   # 模型设置不预测value，则此处仅取1个cell作为column的语义补充
        ...
        return final_tokens, value_list, [table_indexes, column_indexes, value_indexes, num_value_indexes], token_idx_mapping
