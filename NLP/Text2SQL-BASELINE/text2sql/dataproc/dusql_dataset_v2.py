    ...
    with open(schema_file) as ifs_schema, open(content_file) as ifs_content:
        ...
        for schema_dict in lst_schema:
            ...
            tables = tuple(
                ...
                for i, (name, orig_name) in enumerate(zip(schema_dict['table_names'], schema_dict['table_names_original']))
            )
            columns = tuple(
                Column(
                    id = i,
                    ...
                    # 1. 长度大于 20 的丢弃
                    # 2. item_ 开头的是 ID 类
                    cells = [x for x in set([str(c) for c in lst_column_cells[i]]) if len(x) <= 20 or x.startswith('item_')],
                )
                for i, ((table_id, col_name), (_, orig_col_name), col_type) in enumerate(zip(
                    schema_dict['column_names'], schema_dict['column_names_original'], schema_dict['column_types']))
            )
...
class DuSQLExample(object):
    ...
    def _compute_relations(self):
        ...
        link_info_dict = {'sc_link': schema_linking_results,
        ...
        relation_matrix = linking_utils.build_relation_matrix(link_info_dict,
                                                              total_len,
                                                              q_len,
                                                              c_len,
                                                              list(range(c_len + 1)),
