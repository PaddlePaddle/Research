...
def inference(model, data, output_path, beam_size=1, mode='infer', output_history=True, use_heuristic=True, model_name='seq2tree'):
    model.eval()
...
def _do_infer(model, data, beam_size, output_history, ofs, use_heuristic=True, model_name='seq2tree'):
    for i, (inputs, labels) in enumerate(tqdm.tqdm(data())):
        ...
        lst_output = [
                ...
                pred_query,
                db_id,
                json.dumps({'db_id': db_id,
                            'question_id': question_id,
                            'question': question,
                            'gold_query': gold_query,
                            'values': values,
                            'beams': decoded}, ensure_ascii=False),
                ]
